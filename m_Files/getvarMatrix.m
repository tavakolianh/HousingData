...
function [V]=getvarMatrix(x,param,ar,k,distrib,std_method)
...
for i=1:numel(param)
    ...
    [sumlik1,Output,logLikVec1]=MS_AR_Lik(x,param,ar,k,distrib,disp);
    [sumlik2,Output,logLikVec2]=MS_AR_Lik(x,m,ar,k,distrib,disp);

    s(:,i)=(logLikVec2-logLikVec1)/(2*myDelta(i));   %M   size=(n*numel(param))

end

sum_s_Matrix=zeros(numel(param));                    %M   size=(numel(param)*numel(param))
for idx=1:n
    s_Matrix{idx,1}=s(idx,:)'*s(idx,:);              %M   size=(numel(param)*numel(param))
...
for i=1:numel(param)
    ...
    for j=1:numel(param)
        ...
        [sumlik1]=MS_AR_Lik(x,param,ar,k,distrib,disp);
        [sumlik2]=MS_AR_Lik(x,m1,ar,k,distrib,disp);
        [sumlik3]=MS_AR_Lik(x,m2,ar,k,distrib,disp);
